...
description: Bài viết này cung cấp thông tin về cách tạo và xác nhận các tạp chí Entry trong Microsoft Dynamics 365 Project Operations.
author: rumant
ms.date: 09/18/2020
ms.topic: article
ms.prod: ''
...
1. Đi đến **Việc bán hàng** \> **Giao dịch** \> **Tạp chí**.
2. Trên **Tạp chí đầu vào** trang danh sách, trên Ngăn Hành động, hãy chọn **Mới** để tạo một nhật ký.
3. Trên **Tạp chí mới** trang, trong **Sự mô tả**, nhập mô tả của tạp chí.
4. Đảm bảo rằng **Loại tạp chí** trường được đặt thành **Lối vào**, và sau đó chọn **Tiết kiệm**. Sau khi tạp chí Entry mới được lưu, **Dòng nhật ký** tab sẽ xuất hiện trên trang tạp chí.
5. Trên **Dòng nhật ký** trên thanh công cụ phía trên lưới, hãy chọn **Mới** để tạo một dòng nhật ký Entry.
6. Bên trong **Tạo nhanh** hộp thoại để tạo dòng nhật ký Nhập, đặt các trường như được mô tả trong bảng sau.
    ...
    | Mô tả bên ngoài | Mô tả dòng tạp chí có thể được sử dụng để chia sẻ với các bên liên quan bên ngoài. | Đối với các dòng nhật ký bán hàng chưa lập hóa đơn, giá trị sẽ được sử dụng làm mô tả bên ngoài khi tạo chi tiết dòng hóa đơn. Nó cũng có thể xuất hiện trên hóa đơn được gửi cho khách hàng. |
    ...
    | Ngày bắt đầu | Sử dụng ngày khi giao dịch xảy ra. | Trường này được sử dụng để so sánh với ngày tạo hóa đơn cho các giao dịch của **Bán hàng chưa lập hóa đơn** loại hình. So sánh này sẽ giúp bạn quyết định xem giao dịch là ngày trong tương lai hay quá khứ. Chỉ các giao dịch ngày trước mới được thêm vào hóa đơn. |
    ...
    | Phương thức số tiền | Theo mặc định, trường này được đặt thành **Nhân số lượng với giá**. Khi phương pháp này được sử dụng, số tiền sẽ được tính là *Số lượng* ×*Giá bán*. Phương pháp được hỗ trợ khác là **Giá cố định**. Khi phương pháp này được sử dụng, giá sẽ được đặt thành số tiền và số lượng sẽ không được sử dụng trong tính toán. | |
